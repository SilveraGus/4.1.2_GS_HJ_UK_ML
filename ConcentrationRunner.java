...
public class ConcentrationRunner
{
    ...
    public static void displayBoard() {
        ...
        System.out.print('\u000C' + " ");
        for(int i = 0; i < game.gameboard[0].length-1; i++)
            System.out.print("_____");
        System.out.println("________");//final prinln
        for(int i = 0; i < game.gameboard.length; i++){
            for(int j = 0; j < game.gameboard[i].length; j++){
                if(game.gameboard[i][j].isFaceUp()){
                    System.out.print("|"+game.gameboard[i][j].getFace());
                }
                else{
                    System.out.print("|"+game.gameboard[i][j].getBack());
                }
            }
            System.out.println("|");
        }
     ...
     */
    private static void wait(double n) {
        try  {
            Thread.sleep((int)(n * 1000.0));
        }
